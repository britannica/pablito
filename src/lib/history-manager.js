...
class HistoryManager {
  ...
  pushPropertyChanges(changes) {
    ...
    var isValid = changes.every(change => {
      return (
        change.objectIndex !== undefined &&
        change.property !== undefined &&
    ...
    const processChange = currentChange => {
      return new Promise((resolve, reject) => {
        if (currentChange.type === "add") {
          ...
          if (object === undefined) {
            var message =
              "Attempted to retrieve object " +
              currentChange.data.objectIndex +
              " but it's not there";
